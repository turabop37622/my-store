      ...
      <section className="mx-auto max-w-[1600px] px-4 md:px-10 py-12">
        <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4 mb-8">
          <div>
            ...
            <h2 className="text-3xl md:text-5xl font-black mt-2 uppercase tracking-tighter">
              Most loved <span className="text-primary/80">this week</span>
            </h2>
          </div>
...
function HeroCarousel({ banners, loading }: { banners: Product[]; loading: boolean }) {
  ...
  const slides = [
    { img: hero2, upto: "65", label: "BreezyGo", tag: "NEW", price: "4,499 PKR", color: "#0f172a" },
    { img: hero3, upto: "50", label: "BreezyGo", tag: "HOT", price: "1,999 PKR", color: "#171717" },
  ];
        ...
        {slides.map((slide, i) => (
          ...
          >
            <div className={`w-full h-full transition-transform duration-[5000ms] ease-linear ${i === current ? "scale-110" : "scale-100"}`}>
              <img src={slide.img} alt={`Banner ${i + 1}`} className="w-full h-full object-cover" />
            </div>
