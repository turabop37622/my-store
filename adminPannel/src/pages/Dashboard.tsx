...
export default function Dashboard() {
  ...
  if (!data) return <div className="p-10 text-red-500">Failed to load stats. Ensure backend server is running.</div>;

  const stats = [
    { label: "Total Revenue", value: `Rs ${(data.totalRevenue || 0).toLocaleString()}`, icon: DollarSign, color: "text-emerald-500", bg: "bg-emerald-50" },
    { label: "Total Orders", value: data.totalOrders.toString(), icon: ShoppingCart, color: "text-blue-500", bg: "bg-blue-50" },
    { label: "Products", value: data.totalProducts.toString(), icon: Package, color: "text-purple-500", bg: "bg-purple-50" },
    { label: "Messages", value: data.totalMessages.toString(), icon: MessageSquare, color: "text-amber-500", bg: "bg-amber-50" },
  ];
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-gradient-to-br from-emerald-500 to-emerald-600 text-white p-6 rounded-2xl shadow-lg">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-sm font-bold uppercase tracking-wider opacity-70">Today's Revenue</span>
          </div>
        ...
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 text-white p-6 rounded-2xl shadow-lg">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-sm font-bold uppercase tracking-wider opacity-70">Today's Orders</span>
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat) => (
          <div key={stat.label} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
            <div className={`h-12 w-12 rounded-full flex items-center justify-center ${stat.bg} ${stat.color} shrink-0`}>
              <stat.icon className="h-5 w-5" />
            </div>
            <div className="min-w-0">
              <p className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider truncate">{stat.label}</p>
